...
const refreshToken = async (token: string) => {
  ...
  try {
    verifiedToken = verifyToken(token, configs.jwt_refresh_secret as Secret);
  } catch (err: any) {
    console.log(err);
  ...
  const jwtPayload: IJwtPayload = {
    ...
    hasCharity: isUserExist.hasCharity,
...
const forgotPassword = async ({ email }: { email: string }) => {
  ...
  const otpToken = jwt.sign({ otp, email }, configs.jwt_otp_secret as string, {
    expiresIn: '5m',
  });
...
const verifyOTP = async ({ email, otp }: { email: string; otp: string }) => {
  ...
  const decodedOtpData = verifyToken(
    ...
    configs.jwt_otp_secret as string,
  );
  ...
  const resetToken = jwt.sign(
    { email },
    configs.jwt_pass_reset_secret as string,
    {
      expiresIn: configs.jwt_pass_reset_expires_in,
    },
  );
